...
// A larger value might be used to slow down the I²C communication
...
bool OLED::i2c_send(uint8_t byte)
{
    for (int_fast8_t bit = 7; bit >= 0; bit--)
    {
        ...
        //delayMicroseconds(1); // For data setup time, obviously not needed
#if !defined OUTPUT_OPEN_DRAIN
        pinMode(scl_pin, INPUT);
#endif
        digitalWrite(scl_pin, HIGH);        
...
{        
    ...
    int_fast16_t dx = abs(static_cast<int_fast8_t>(x1) - static_cast<int_fast8_t>(x0));
