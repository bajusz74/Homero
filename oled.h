...
/**
 * Driver fro OLED displays with SSD1306 or SH1106 controller.
 * 
 * Supported display sizes: 128x32, 128x64 and 96x16 pixels.
 ...
 * For a 128x64 display the memory contains 4 pages each of 128 bytes. 
...
public:
     ...
     */
    OLED(uint8_t sda_pin=4, uint8_t scl_pin=5, uint8_t reset_pin=NO_RESET_PIN, uint8_t i2c_address=0x3C, 
    ...
    /**
     * Enable continuous scrolling. This is performed by the display controller itself without needing continuous data 
     * transfer and it does not affect the buffer memory.
     ...
     * But eye reacts logarithmically. A medium brightnes of 128 looks almost as bright as the
     ...
     * @param delay_ms Delay time between each step, recommended values are either 0 or 20..50.
     */
    void scroll_up(uint_fast8_t num_lines=8, uint_fast8_t delay_ms=0);
    
    /**
     * Transfer the buffer memory data to the display controller.
     * You have to call this method after any drawing to make it visible.
     * A call to this method takes ~50ms for the 128x32 display and ~100ms for the 128x64 display.
     */
    void display();    
